...
`npm test` builds the application, runs `tests/rendered-html.test.mjs` under `node --test` (2 tests), then runs `tests/backend-workflow.test.mjs`, `tests/gap-closure.test.mjs`, `tests/reasoning.test.mjs`, and `tests/practice-mode.test.mjs` under `tsx --test` (15 + 11 + 7 + 7 = 40 tests). **42 tests in total.**
