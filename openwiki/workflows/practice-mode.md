...
`PracticeTour` is an 8-stop docked panel: intake → research-canvas → call-kit → transcript → synthesis → diagnosis → deliver → report. Each stop carries a one-paragraph `blurb` and a short `points` list — an operating manual, not a training course. The tour deliberately skips the Prepare and Call screens, which remain reachable outside it. While the walkthrough is open the research screen renders its canvas tab from `research.facts` only (`firstPass`), because at that point in the story no call has happened yet — and it is open for the whole practice session, since `tourMode` only returns to `off` when the advisor leaves practice mode. The same condition locks intake. Position persists in `localStorage`, and on completion the advisor can walk it again, reset the data, leave practice mode, or delete the practice engagement.
